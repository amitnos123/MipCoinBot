...
module.exports = {
    ...
    description: 'Rewrite character\'s bio',
    help: '__Input__' +
        '\n' +
        '[character name]' +
        '\n' +
        ...
        '\n' +
        '`[Bio]` - Can be any length that discord allows',
    args: true,
        ...
        if (args.length < 2) {
            ...
            if (bioMan.character_exists(args[0])) {
                ...
                if (bioMan.get(args[0]).owner === message.author.username || authComManager.is_root(message.author.username)) {
                    ...
                    bioMan.character_update(args[0], newBio);
                    message.channel.send(`Character ${args[0]} bio was rewritten`);
                }
            } else {
